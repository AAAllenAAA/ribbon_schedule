...
def prepare_final_schedule(df_paired_split, df_no_pair_split, df_no_pair_second):
    # 建立主工單欄位
    ...
    # 合併資料
    combined_df = pd.concat([df_paired_split, df_no_pair_split, df_no_pair_second], ignore_index=True)
...
# 全部排完才刪選時間 或是 刪除多餘欄位
def do_date_choose_backup(final_output, df_remaining_second):
    ...
    return final_output, df_remaining_second
...
def main():
    ...
    # 沒啥作用 -> 把剩餘工單的刀次改為""
    final, remaining = do_date_choose_backup(final_output, df_reamining_second) 

    # 工單分配
    A159_part, A830_part, B201_part = do_people(final)
    ...
    output_name = os.path.join(script_dir, "第一版排程結果_0717.xlsx")   # 存到同一層資料夾
    ...
    with pd.ExcelWriter(output_name, engine="openpyxl") as writer:
        #df_paired.to_excel(writer, sheet_name="第一次處理前配對", index=False)
        #df_paired_split.to_excel(writer, sheet_name="第一次處理後配對", index=False)
        #df_remaining.to_excel(writer, sheet_name="第一次處理後剩餘", index=False)
        #df_no_pair_second.to_excel(writer, sheet_name="第二次處理後配對", index=False)
        #df_reamining_second.to_excel(writer, sheet_name="第二次處理後剩餘", index=False)
        #final.to_excel(writer, sheet_name="最初結果", index=False)
