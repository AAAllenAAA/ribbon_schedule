...
def sort_by_customer_due_date(df: pd.DataFrame, break_dates: set = None) -> pd.DataFrame:
    ...
    # ------------------------------------------------------------------
    history_df = df[df['is_history_job'] == 'TRUE'].copy()
    new_df = df[df['is_history_job'] == 'FALSE'].copy().reset_index(drop=True)
...
def final_schedule_list_second(A159_part, A830_part, B201_part, break_dates):
    ...
    return schedule_A830, schedule_A159_3, schedule_B201_3, schedule_A159_1, schedule_A159_2
...
def main():
    ...
    A830_part_end_list, A159_part_end_list, B201_part_end_list, A159_1, A159_2 = final_schedule_list_second(A159_part, A830_part, B201_part, break_dates)
    ...
    # ========================================================================
    # === 新增步驟：呼叫 AI 模型腳本 (排程優化) ===
    # ========================================================================
    
    # 1. 配置模型腳本路徑 (請依您的實際位置調整)
    # 假設 predict_AI_result.py 放在同一個資料夾，或指定固定路徑
    MODEL_SCRIPT_PATH = r"E:\ribbon_schedule\training_AI\ML\predict_AI_result.py"
    
    # 2. 預期 AI 優化後 Excel 的輸出路徑
    # 必須與 predict_AI_result.py 內部動態生成 OUTPUT_RESULT_PATH 的邏輯一致 (加 _ML_RESULT)
    base_name = os.path.splitext(output_name)
    optimized_output_name = base_name[0] + "_ML_RESULT" + base_name[1] 

    final_output_path = output_name # 預設是 Touring 結果路徑 (如果 AI 失敗，返回此路徑)
    
    try:
        # 執行命令只傳入一個參數：Touring 結果路徑 (output_name)
        cmd = [
            sys.executable,        # 確保使用當前的 Python 解釋器
            "-X", "utf8",
            MODEL_SCRIPT_PATH,
            output_name,           # 參數 1: 輸入檔案路徑 (Touring 結果)
        ]
        
        print(f"--- 啟動 AI 優化腳本: {MODEL_SCRIPT_PATH} ---")
        
        # 執行腳本，並等待其完成，捕獲輸出。check=True 會在模型腳本非正常退出時拋出錯誤
        result = subprocess.run(cmd, capture_output=True, text=True, check=True, encoding='utf-8')
        #result = subprocess.run(cmd, capture_output=True, text=True, check=True, encoding='cp950')
        
        print("✅ 模型優化腳本執行成功。")
        
        # 3. 檢查模型腳本回傳的 FINAL_PATH
        # 使用正規表達式查找模型腳本最後打印的 FINAL_PATH:
        match = re.search(r"FINAL_PATH:(.*)", result.stdout)
        
        if match:
            # 如果找到，使用模型腳本回傳的實際路徑
            final_output_path = match.group(1).strip()
            print(f"使用模型回傳路徑: {final_output_path}")
        else:
            # 如果模型成功，但未打印 FINAL_PATH，則使用預期的路徑
            final_output_path = optimized_output_name 
            print(f"模型未明確回傳路徑，使用預期路徑: {final_output_path}")
            
        print("模型輸出日誌 (stdout):\n" + result.stdout) # 打印所有模型日誌
            
    except FileNotFoundError:
        print(f"❌ 致命錯誤: 找不到模型腳本 {MODEL_SCRIPT_PATH}。返回未優化結果。")
    except subprocess.CalledProcessError as e:
        print(f"❌ 錯誤: 模型腳本執行失敗 (非零退出碼)。錯誤訊息 (stderr):\n {e.stderr}")
        # 模型失敗時，final_output_path 保持為 output_name (Touring 原始結果)
    except Exception as e:
        print(f"❌ 錯誤: 執行模型時發生未知錯誤: {e}。返回未優化結果。")
...
if __name__ == '__main__':
    excel_path, final_output_path = main()
    #excel_path = main()
    ...
    # 寫入 txt 給 PHP
    path_file = "E:/ribbon_schedule/test_report_upload/json/excel_path.txt"
    with open(path_file, "w", encoding="utf-8") as f:
        f.write(final_output_path)
        #f.write(excel_path)
