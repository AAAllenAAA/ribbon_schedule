...
class DateFilterDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.end_entry = DateEntry(self, year=2025, month=12, day=1, **dateentry_style) # test use
...
def process_schedule_data():
    ...
    # 步驟 A: 歷史資料扣除並更新電子報
    # 如果有之前多做先配對入庫的
    # 先判斷歷史資料此工單是否有排過 如果沒有 且 完工數量不為0那 df["開工數量"] - df["完工數量"]
    # ==========================================================
    df = merged.copy()
    ...
    if not df_history.empty:
        ...
        # 扣除歷史數量
        df['開工數量'] = np.where(
            (df['昨日已排數量'] == 0) & (df['完工數量'] > 0),
            (df['開工數量'] - df['完工數量']).clip(lower=0),
            (df['開工數量'] - df['昨日已排數量']).clip(lower=0)
        )
...
def prepare_final_schedule(all, final_remaining, df_history):
    ...
    try:
        if "GLOBAL_ORDER_QTY_MAP" in globals():
            ...
            for order_id, planned_qty in GLOBAL_ORDER_QTY_MAP.items():
                ...
                # 計算 combined_df 的總餘量
                total_staff = combined_df.loc[
                    combined_df["工單編號"] == order_id, "餘量"
                ].sum()

                total_actual = total_staff
...
#--------------------日期排程----------------------------        
def final_schedule_list(A159_part, A830_part, B201_part, df_history):
    ...
    #schedule_A159, schedule_B201, schedule_A830 = schedule_history_download(schedule_A830, schedule_B201, schedule_A159, df_history)
    ...
    if not schedule_B201.empty and schedule_B201["刀次"].notna().any() and (schedule_B201["刀次"] > 0).any():
        ...
        # 丟進排程邏輯（使用 datetime 版本）
        try:
            schedule_B201_1 = final_cal_list(schedule_B201, start_d=schedule_start, break_dates=break_dates["B201"])
            schedule_B201_2 = sort_by_customer_due_date(schedule_B201_1)
            schedule_B201_3 = final_cal_list(schedule_B201_2, start_d=schedule_start, break_dates=break_dates["B201"])
        except Exception as e:
            print("發生錯誤:", e)
    ...
    return schedule_A830, schedule_B201_3, schedule_A159
...
def final_doAssignAndSort(df_paired_split, df_no_pair_split, df_no_pair_second, remaining):
    ...
    # 先去"基本資料"中找到其"搭一料號" 再去資料中搜尋是否有符合的
    ...
    for idx, row in remaining.iterrows():
        ...
        print(f"\n[處理] {idx+1}/{len(remaining)} | 工單號: **{sn}** | 品號: **{original_item}** | 剩餘量: {original_qty}")

        # 找所有符合的基本資料
        match_rows = base_df[base_df["料號"] == original_item]
        if match_rows.empty:
            print(f"   [BaseInfo] 🚫 找不到對應基本資料，略過。")
            final_rows.append(original_order)
            continue

        main_car_count = pd.to_numeric(original_order.get("車數", 1), errors="coerce") or 1
        original_qty = pd.to_numeric(original_qty, errors="coerce")
        if main_car_count <= 0:
            main_car_count = 1

        if pd.isna(original_qty) or pd.isna(main_car_count):
            main_cut_count = 0
            print(f"⚠️ 警告：工單 {sn} 缺少數量資料，以 0 處理。")
        else:
            main_cut_count = math.ceil(original_qty / main_car_count)

        original_order["刀次"] = main_cut_count
        print(f"   [Cut] 計算所需刀次: {main_cut_count} (良品數:{original_qty} / 車數:{main_car_count})")

        # 遍歷所有可能的搭配組合（同料號多筆）
        paired_found = False

        for i, m in match_rows.iterrows():
            if paired_found:
                break  # 外層迴圈停止，避免同料號第二筆被處理
            ...
            print(f"   [BaseInfo] ✅ 嘗試搭配組合: 搭1:{d1_code}({d1_car_count}) | 搭2:{d2_code}({d2_car_count})")

            # 嘗試從 df_no_pair_split / second 找搭配
            for df_src, src_name in [(df_no_pair_split, "df_no_pair_split"),
                                    (df_no_pair_second, "df_no_pair_second")]:
                if df_src is not None:
                    match_in_df = df_src[df_src["品號"] == d1_code]
                    if not match_in_df.empty:
                        paired_row = match_in_df.iloc[0].copy()
                        paired_found = True  # 標記已找到搭配
                        print(f"   [Match] 💡 從 {src_name} 找到可直接配對工單: "
                            f"{paired_row.get('工單編號', 'N/A')} ({d1_code})")
                        ...
                        break  # 跳出 no_pair 來源迴圈


        # 若前面沒找到，也照原邏輯跑
        final_rows.append(original_order)
        process_pair_order(d1_code, d1_car_count, main_cut_count, all_staff_df,
                           original_order, final_rows, width_map,
                           df_paired_split=df_paired_split,
                           df_no_pair_split=df_no_pair_split,
                           df_no_pair_second=df_no_pair_second)
        process_pair_order(d2_code, d2_car_count, main_cut_count, all_staff_df,
                           original_order, final_rows, width_map,
                           df_paired_split=df_paired_split,
                           df_no_pair_split=df_no_pair_split,
                           df_no_pair_second=df_no_pair_second)
    ...
    # Check重複
    final_all_df = all_staff_df.copy()
    for idx, row in final_df.iterrows(): 
        sn = row["工單編號"] 
        qty = row["預估良品數"] 
        match_in_all = final_all_df[ 
            (final_all_df["工單編號"] == sn) & 
            (pd.to_numeric(final_all_df["預估良品數"], errors="coerce") == qty) 
            ] 
        if not match_in_all.empty: final_all_df.drop(match_in_all.index, inplace=True)

    return {"all": final_all_df.reset_index(drop=True), "final_remaining": final_df}
...
def process_pair_order(pair_code, car_count, main_cut_count,
    ...
    main_order_id = paired_row.get("工單編號", "N/A")

    # 判斷資料來源
    in_no_pair = (
        (df_no_pair_split is not None and main_order_id in df_no_pair_split["工單編號"].values)
        or
        (df_no_pair_second is not None and main_order_id in df_no_pair_second["工單編號"].values)
    )
    in_paired = (df_paired_split is not None and main_order_id in df_paired_split["工單編號"].values)

    # 來源不同 → 處理邏輯不同
    if in_no_pair:
        print(f"   [Match] 💡 工單 {main_order_id} 來自 no_pair，可直接搭配使用。")
        ...
        print(f"   [Match] 🔍 工單 {main_order_id} 來自 paired，檢查可用量：原始 {original_qty}, 已用 {used_qty}, 可用 {available_qty}")

        if available_qty <= 0:
            print(f"   [PairOrder] ⚠️ 可用量不足，建立補空白行。")
            new_row = original_order.copy()
            new_row["工單編號"] = ""
            new_row["品號"] = pair_code
            new_row["車數"] = car_count
        ...
        print(f"   [Match] ⚠️ 工單 {main_order_id} 來源不明，預設按 paired 檢查。")
        original_qty = int(GLOBAL_ORDER_QTY_MAP.get(main_order_id, 0))
        used_qty = all_staff_df.loc[all_staff_df["工單編號"] == main_order_id, "預估良品數"].sum()
        available_qty = original_qty - used_qty
        qty_to_use = min(main_cut_count * car_count, max(available_qty, 0))
...
def merge_order_cutNum(A_df: pd.DataFrame, B_df: pd.DataFrame, C_df: pd.DataFrame,
    ...
    def process_one(df: pd.DataFrame, person_name: str) -> pd.DataFrame:
        ...
        # ===== 新增：初始化每一天的 total_knives 和 existing_keys =====
        daily_status = {}  # key=date, value=dict(total_knives, existing_keys)
        for d in all_dates:
            daily_ods = [od for od in orders if d in od["slices"] and od["slices"][d]>0]
            total_knives = sum(od["slices"][d] for od in daily_ods)
            existing_keys = {get_item_group_key(od["orig_row"].get("品號")) for od in daily_ods}
            daily_status[d] = {"total_knives": total_knives, "existing_keys": existing_keys}
        # ===========================================================
        ...
        for idx, current_date in enumerate(all_dates):
            ...
            # 找未來工單 (僅考慮 all_dates 裡面的日期)
            future_cand = []
            for od in orders:
                for sdate, amt in od["slices"].items():
                    if pd.notna(sdate) and sdate in all_dates[idx+1:] and amt>0:
...
def schedule_history_download(A830_part_end_list, B201_part_end_list, A159_part_end_list, history):
    """
    根據歷史資料，將前一天尚未完成的工單在此次排程中優先排前面。
    只移動主工單刀次 > 0，且包含其緊接的子工單（刀次=0）。
    傳入的 A830_part_end_list, B201_part_end_list, A159_part_end_list 都是 DataFrame。
    """
    ...
    # 處理三個排程清單
    A830_part_end_list = move_previous_day_orders_to_front(A830_part_end_list)
    B201_part_end_list = move_previous_day_orders_to_front(B201_part_end_list)
    A159_part_end_list = move_previous_day_orders_to_front(A159_part_end_list)

    return A830_part_end_list, B201_part_end_list, A159_part_end_list
...
def main():
    ...
    df_paired_split, remaining = remaining_cut_clean_and_repair(df_paired_split_3, df_reamining_second, mode = "88cm") 
    ...
    final_output = prepare_final_schedule(all, final_remaining, df_history)

    # 工單分配
    A159_part, A830_part, B201_part = do_people(final_output)

    # 輸入休假 彈出視窗 選擇人員以及休假日期
    A830_part_end_list_before, B201_part_end_list_before, A159_part_end_list_before = final_schedule_list(A159_part, A830_part, B201_part, df_history)

    # 載入歷史資料 觀察是否有前一日尚未做完的工單 要繼續做完
    A830_part_end_list, B201_part_end_list, A159_part_end_list = schedule_history_download(A830_part_end_list_before, B201_part_end_list_before, A159_part_end_list_before, df_history)
    ...
    # 嘉真需求 補上米平方 & 按週分組
    A830_part_end, B201_part_end, A159_part_end = Erin_use(A830_part_end_1, B201_part_end_1, A159_part_end_1)
    ...
    # ==============================================================================================================
    # 重新邏輯 分類工單
    #final = post_optimization_clean_up(A830_part_end, B201_part_end, A159_part_end)

    #final_81B_assigned, remaining_unassigned = assign_81b_priority_groups(final)

    # === 寫入 Excel ===
    script_dir = os.path.dirname(os.path.abspath(__file__))  # 取得目前.py檔的資料夾
    output_name = os.path.join(script_dir, "排程結果_1110(1111).xlsx")   # 存到同一層資料夾
    ...
    with pd.ExcelWriter(output_name, engine="openpyxl") as writer:
        ...
        #df_paired_split_3.to_excel(writer, sheet_name="check配對結果2", index=False)
        #extra_remaining_2.to_excel(writer, sheet_name="剩餘", index=False)
        ...
        #final_output.to_excel(writer, sheet_name="初步分配人員", index=False)
        ...
        A159_part_end_list_before.to_excel(writer, sheet_name="家偉_merge前", index=False)
        B201_part_end_list_before.to_excel(writer, sheet_name="旺斌_merge前", index=False)
        A830_part_end_list_before.to_excel(writer, sheet_name="容合_merge前", index=False)
        
        A159_part_end_list.to_excel(writer, sheet_name="家偉_merge前_歷史", index=False)
        B201_part_end_list.to_excel(writer, sheet_name="旺斌_merge前_歷史", index=False)
        A830_part_end_list.to_excel(writer, sheet_name="容合_merge前_歷史", index=False)
