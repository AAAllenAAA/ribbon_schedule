...
class DateFilterDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.start_entry = DateEntry(self, year=2025, month=9, day=20, **dateentry_style) # test use
...
def split_paired_rows(df_paired, max_cut=60):
    ...
    return used_df, pd.DataFrame(remaining_rows)
...
def merge_same_day_orders_multi(*dfs: pd.DataFrame) -> Tuple[pd.DataFrame, ...]:
    """
    多個 DataFrame 一次處理
    - 只用主工單判斷是否同一天重複
    - 如果主工單重複，必須連同子工單集合完全一致才合併
    - 子工單可能為空，會統一視為 "EMPTY_SUB"
    - 空 DataFrame 自動跳過
    - '預計開工日' 最後統一格式化為 'yyyy-mm-dd'
    """
    merged_results = []
    ...
    for i, df in enumerate(dfs, start=1):
        ...
        df = df.copy()
        if "預計開工日" not in df.columns:
            raise KeyError(f"第 {i} 個 DataFrame 缺少 '預計開工日' 欄位")
        ...
        for idx, row in df.iterrows():
            ...
            if row["刀次"] > 0:  # 主工單
                # 找到同一天相同主工單
                same_main = df[(df["工單編號"] == main_order_id) &
                               (df["預計開工日"] == work_date) &
                               (df["刀次"] > 0)]
                ...
                if len(same_main) > 1:
                    # 收集每組主工單的子工單集合
                    groups = []
                    for _, main_row in same_main.iterrows():
                        sub_rows = []
                        start_idx = main_row.name + 1
                        while (
                            start_idx < len(df)
                            and df.iloc[start_idx]["刀次"] == 0
                            and df.iloc[start_idx]["預計開工日"] == work_date
                        ):
                            sub_id = df.iloc[start_idx]["工單編號"]
                            if pd.isna(sub_id) or str(sub_id).strip() == "":
                                sub_rows.append("EMPTY_SUB")
                            else:
                                sub_rows.append(str(sub_id))
                            start_idx += 1
                        groups.append(set(sub_rows))

                    # 檢查子工單集合是否完全一致
                    if len(set(map(tuple, groups))) == 1:
                        # 合併主工單
                        merged_main = same_main.iloc[0].copy()
                        merged_main["刀次"] = same_main["刀次"].sum()
                        if "預估良品數" in df.columns:
                            merged_main["預估良品數"] = same_main["預估良品數"].sum()
                        if "餘量" in df.columns:
                            merged_main["餘量"] = same_main["餘量"].sum()
                        merged_rows.append(merged_main)
                        used_idx.update(same_main.index)

                        # 合併子工單
                        first_group = list(groups[0])
                        for sub_id in first_group:
                            if sub_id == "EMPTY_SUB":
                                sub_group = df[(df["刀次"] == 0) &
                                               (df["預計開工日"] == work_date) &
                                               (df["工單編號"].isna() | (df["工單編號"].astype(str).str.strip() == ""))]
                            else:
                                sub_group = df[(df["刀次"] == 0) &
                                               (df["預計開工日"] == work_date) &
                                               (df["工單編號"] == sub_id)]
                            if not sub_group.empty:
                                merged_sub = sub_group.iloc[0].copy()
                                merged_sub["刀次"] = 0
                                if "預估良品數" in df.columns:
                                    merged_sub["預估良品數"] = sub_group["預估良品數"].sum()
                                if "餘量" in df.columns:
                                    merged_sub["餘量"] = sub_group["餘量"].sum()
                                merged_rows.append(merged_sub)
                                used_idx.update(sub_group.index)
                    else:
                        # 子工單集合不一致 → 不合併
                        for _, r in same_main.iterrows():
                            merged_rows.append(r.copy())
                            used_idx.add(r.name)
                else:
...
def main():
    ...
    df_paired_split = split_paired_rows_step1(df_paired_split)
    ...
    df_paired_split, remaining = remaining_cut_clean_and_repair(df_paired_split, df_reamining_second, mode = "88cm") 
    ...
    A830_part_end, B201_part_end, A159_part_end = merge_same_day_orders_multi(A830_part_end, B201_part_end, A159_part_end) 

    # === 寫入 Excel ===
    script_dir = os.path.dirname(os.path.abspath(__file__))  # 取得目前.py檔的資料夾
    output_name = os.path.join(script_dir, "排程結果_0902.xlsx")   # 存到同一層資料夾
    ...
    with pd.ExcelWriter(output_name, engine="openpyxl") as writer:
        #df_paired_split.to_excel(writer, sheet_name="初始剩餘", index=False)
        #df_no_pair_split.to_excel(writer, sheet_name="測試1", index=False)
        #df_no_pair_second.to_excel(writer, sheet_name="測試2", index=False)
        #A159_part.to_excel(writer, sheet_name="家偉_第一階段", index=False)
        #B201_part.to_excel(writer, sheet_name="旺斌_第一階段", index=False)
        #A830_part.to_excel(writer, sheet_name="容合_第一階段", index=False)
