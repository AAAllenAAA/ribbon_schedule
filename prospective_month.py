...
def apply_final_rotation(df_A830, df_B201, df_A159):
    ...
    【身分互換輪替版】
    邏輯：
    1. 算出工單所屬週次（以 12/16 為基準週 0）。
    2. 定義當週「值班生」(Standby Person)：負責執行 A159 的任務 (81B)。
    3. 如果當週值班生不是 A159：
       - 原本分給 A159 的工單 -> 轉交給該值班生。
       - 原本分給該值班生的工單 -> 轉交給 A159 代班。
    4. 如此循環，確保每週只有一位專員執行 81B，且 A159 在非值班週會去接手別人的工作。
    """
    # 合併所有資料進行標籤重整
    full_df = pd.concat([df_A830, df_B201, df_A159], ignore_index=True)
    if full_df.empty:
    ...
    # 1. 建立時間基準 (以您指定的 12/16 為週序 0)
    full_df['預計開工日_dt'] = pd.to_datetime(full_df['預計開工日'], errors='coerce')
    reference_date = pd.Timestamp('2024-12-30')
    
    # 計算每一列屬於第幾週 (// 7 確保得到整數索引)
    full_df['週序'] = (full_df['預計開工日_dt'] - reference_date).dt.days // 7
    
    # 2. 鎖定工單首日週序 (避免同一張工單跨週時被分配給不同人)
    ...
    def swap_identity(row):
        try:
            ...
            # 如果當週值班生就是 A159，則按原定計畫執行，不需交換
            if standby_person == "A159":
                return current_p
            
            # --- 核心交換邏輯 ---
            # A. 原本給 A159 的 (81B) -> 交給當週值班生
            if current_p == "A159":
                return standby_person
            
            # B. 原本給該值班生的 (81A) -> 移交給 A159 代班
            if current_p == standby_person:
                return "A159"
            
            # C. 其他人員 (非值班生也非 A159) 保持原樣
            return current_p
        except:
            return row['人員']
    ...
    # 5. 【穩定排序】確保主子工單不失散
    # 先把原始的順序記下來（建立一個臨時的索引欄位）
    full_df['original_index'] = range(len(full_df))
    
    # 排序條件：1. 日期 (A到Z)  2. 原始順序 (確保子工單跟在主工單後面)
    full_df = full_df.sort_values(by=['預計開工日', 'original_index'], ascending=[True, True])

    # 6. 清理輔助欄位並拆分回傳
    full_df.drop(columns=['預計開工日_dt', '週序', 'original_index', '主工單總數量', '優化後順序', 'is_history_job_cleaned', '主工單編號', '來源類型', '週分組'], inplace=True, errors='ignore')
...
def fill_standby_capacity_with_limit(df_A830, df_B201, df_A159):
    """
    【完全自動化版】
    1. 自動計算「今天」是誰值班。
    2. 參考 MAX_KNIFE_MAP 補足該員產能。
    """
    import pandas as pd
    ...
    # 4. 統計該員在今天的初始狀態
    person_mask = (full_df['預計開工日_dt'] == today_dt) & (full_df['人員'] == standby_person)
    daily_jobs = full_df[person_mask]
    
    current_knives = daily_jobs['刀次'].sum()
    # 使用你的 get_item_group_key 邏輯 (簡化為前三碼)
    existing_keys = {str(item)[:3] for item in daily_jobs['料號'].unique() if pd.notna(item)}
    
    # 初始上限
    num_items = len(existing_keys)
    day_limit = MAX_KNIFE_MAP.get(num_items, 39 if num_items > 0 else 65)

    # 5. 補單邏輯：僅補足今天的 standby_person
    if current_knives < day_limit:
        # 找未來：人員標籤已確定是 standby_person 且日期在今天之後
        future_mask = (full_df['預計開工日_dt'] > today_dt) & (full_df['人員'] == standby_person)
        future_candidates = full_df[future_mask].copy().sort_values('預計開工日_dt')

        for idx, row in future_candidates.iterrows():
            pick_key = str(row['料號'])[:3]
            pick_knives = row['刀次']
            
            # 即時計算加入這筆後的品號種類與新上限
            temp_keys = existing_keys | {pick_key}
            new_limit = MAX_KNIFE_MAP.get(len(temp_keys), 39)
            
            # 檢查是否超過動態上限
            if (current_knives + pick_knives) <= new_limit:
                # 執行「前拉」
                full_df.at[idx, '預計開工日_dt'] = today_dt
                full_df.at[idx, '預計開工日'] = today_dt.strftime('%Y-%m-%d')
                
                # 更新統計數據，繼續下一筆檢查
                current_knives += pick_knives
                existing_keys.add(pick_key)
            else:
                # 塞不下了，停止補單 (或是跳過找下一張更小的)
                continue

    # 6. 拆分回傳
    return (full_df[full_df['人員'] == 'A830'].copy(), 
            full_df[full_df['人員'] == 'B201'].copy(), 
            full_df[full_df['人員'] == 'A159'].copy())




def final_clean_and_reorder(df, order_df):
    if df.empty:
        return df

    # --- 步驟 1: 基礎處理 ---
    df['預估良品數'] = pd.to_numeric(df['預估良品數'], errors='coerce').fillna(0)
    ...
    def format_date(x):
        return f"{x.year}/{x.month}/{x.day}" if pd.notnull(x) else ""

    df['主工單最早開工'] = df['主工單最早開工'].apply(format_date)
    ...
    # --- 步驟 2: 處理子工單與橫向展開 (為了找出誰是搭接單) ---
    sub_df = df[df['刀次'] == 0].copy()
    if not sub_df.empty:
        sub_df['工單編號'] = sub_df['工單編號'].fillna("").astype(str)
        # 這裡我們要計算：每一張「工單」作為「子工單」時，總共佔用了多少數量
        # 這是一個全局的統計，不分組
        global_sub_usage = sub_df.groupby('工單編號')['預估良品數'].sum().to_dict()
        
        # 繼續原本的 pivot 邏輯
        sub_pivot_data = sub_df.groupby(['唯一主工單ID', '工單編號', '品號'], as_index=False).agg({'預估良品數': 'sum'})
        sub_pivot_data['sub_rank'] = sub_pivot_data.groupby('唯一主工單ID').cumcount() + 1
        pivot_subs = sub_pivot_data.pivot(index='唯一主工單ID', columns='sub_rank', values=['工單編號', '品號', '預估良品數'])
        pivot_subs.columns = [f'搭{int(rank)}{label}' for label, rank in pivot_subs.columns]
        pivot_subs = pivot_subs.rename(columns=lambda x: x.replace('工單編號', '工單').replace('品號', '料號').replace('預估良品數', '數量'))
        pivot_subs = pivot_subs.reset_index()
    else:
        global_sub_usage = {}
        pivot_subs = pd.DataFrame(columns=['唯一主工單ID'])
    ...
    if order_df is not None and not order_df.empty:
        ...
        main_df['開工數量'] = pd.to_numeric(main_df['開工數量'], errors='coerce').fillna(0)
        main_df['完工數量'] = pd.to_numeric(main_df['完工數量'], errors='coerce').fillna(0)
    else:
        main_df['開工數量'] = 0
        main_df['完工數量'] = 0

    # --- 步驟 4: 合併 ---
    final_df = pd.merge(main_df[['唯一主工單ID', '工單編號', '品號', '客戶需求日', '開工數量', '完工數量', '主工單總數量', '主工單最早開工', '主工單最晚完工']], 
                        pivot_subs, on='唯一主工單ID', how='left')

    # --- 步驟 5: 【核心變更】全局扣除邏輯 ---
    # 這裡我們不看這一行的「搭1數量」，而是看這一行的「工單編號」在全局中作為子單被用了多少
    # 使用 map 將 global_sub_usage 映射回主工單
    final_df['被搭接總量'] = final_df['主工單號' if '主工單號' in final_df.columns else '工單編號'].map(global_sub_usage).fillna(0)

    # 預估良品 = 開工數量 - 被搭接總量 - 完工數量
    final_df['預估良品'] = final_df['開工數量'] - final_df['被搭接總量'] - final_df['完工數量']
    ...
    for i in range(1, 6): 
        for suffix in ['工單', '料號', '數量']:
            col_name = f'搭{i}{suffix}'
            if col_name in final_df.columns: partner_cols.append(col_name)

    final_df = final_df[[c for c in (base_cols + partner_cols) if c in final_df.columns]]

    return final_df.sort_values(by='主工單號', ascending=True)


def highlight_needs_supplement(val):
    try:
        num = float(val)
        if num > 0:
            # 分號結尾要確實
            return 'background-color: yellow; color: #FF0000; font-weight: bold;'
    except:
        pass
...
def main():
    ...
    all_final = pd.concat([A830_new, B201_new, A159_new], ignore_index=True)

    all_final_combined = final_clean_and_reorder(all_final, order_df)

    all_final = all_final.drop(columns=['唯一主工單ID', '人員', '主工單總數量', '主工單最早開工', '主工單最晚完工'])

    styled_df = all_final_combined.style.applymap(highlight_needs_supplement, subset=['須補數量'])

    #A159_new_rebalance = reorder_main_and_subs(A159_new_rebalance)
    #A159_new_rebalance = merge_same_day_orders_multi(A159_new_rebalance)
    # ==============================================================================================================
    # === 寫入 Excel ===
    # 1. 準備統計資料 (計算 user_schedule_date 當天的刀次)
    # 將所有人員的 final 結果合併，以便統一計算
    ...
    output_name = os.path.join(script_dir, data_folder, f"ribbon_month_{today}.xlsx")  # 存到同一層資料夾
    ...
    with pd.ExcelWriter(output_name, engine="openpyxl") as writer:
        ...
        #sorted_df_end.to_excel(writer, sheet_name="預排結果", index=False)
        '''
        ...
        styled_df.to_excel(writer, sheet_name="多切", index=False)

        all_final.to_excel(writer, sheet_name="Data", index=False)
    ...
    #return output_name, final_output_path
    return output_name
...
if __name__ == '__main__':
    ...
    json_path = os.path.join(json_dir, "result_info_month.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(result_info, f, ensure_ascii=False, indent=4)

    path_file = os.path.join(json_dir, "excel_path_month.txt")
