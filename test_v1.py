...
class DateFilterDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.start_entry = DateEntry(self, year=2025, month=9, day=19, **dateentry_style) # test use
...
def reorder_main_and_subs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    while i < len(df):
        row = df.iloc[i].copy()
        current_cut = int(row.get("刀次", 0))
        current_car = int(row.get("車數", 0))
        ...
        if current_cut > 0:
            ...
            # 找子工單
            j = i + 1
            while j < len(df) and int(df.iloc[j]["刀次"]) == 0:
                sub_row = df.iloc[j].copy()
                sub_car = int(sub_row.get("車數", 0))
...
def merge_order_cutNum(A_df: pd.DataFrame, B_df: pd.DataFrame, C_df: pd.DataFrame,
    ...
    def process_one(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, row in df.iterrows():
            if int(row.get("刀次", 0)) > 0:
                last_main = {
                    "orig_idx": i,
                    "orig_row": row.copy(),
                    "orig_date": pd.to_datetime(row["預計開工日"]),
                    "remain": int(row["刀次"]),
                    "car": int(row.get("車數", 0)),
                    "children": []
                }
        ...
        for od in orders:
            ...
            if od["remain"] > 0 and pd.notna(od["orig_date"]):
                od["slices"][od["orig_date"].normalize()] = od["remain"]
            ...
            while available > 0:
                ...
                for od in orders:
                    for sdate, amt in od["slices"].items():
                        if pd.notna(sdate) and sdate > date and amt > 0:
        ...
        for od in orders:
            for sdate, amt in sorted(od["slices"].items(), key=lambda x: x[0]):
                if amt > 0:
                    ...
                    main_row["刀次"] = int(amt)
                    main_car = int(main_row.get("車數", 0))
...
def main():
    ...
    output_name = os.path.join(script_dir, "排程結果_0919.xlsx")   # 存到同一層資料夾
