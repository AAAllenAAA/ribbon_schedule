...
class DateFilterDialog(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        #self.start_entry = DateEntry(self, year=2025, month=9, day=17, **dateentry_style) # test use
...
def split_no_pair_rows(df_no_pair, max_knife=45):
    rows = []
...
def split_paired_rows(df_paired, max_cut=45):
...
# 不會有remaining部份
def split_paired_rows_step1(df_paired, remaining, max_cut=45):

    df_paired = df_paired.copy().reset_index(drop=True)
    paired_rows = []
    remaining_rows = []

    original_qty_map = df_paired.groupby("工單編號")["預估良品數"].sum().astype(float).to_dict()
    ...
    new_quantity_remainders = [] # 儲存因短缺產生的剩餘工單
    
    for key, original_qty in original_qty_map.items():
        # 查詢這個工單在拆分後排了多少 (已排程的數量)
        used_qty = used_df[used_df["工單編號"] == key]["預估良品數"].sum()
        
        # 檢查是否短缺: 數量比原本數量少的 (remain_qty > 0)
        remain_qty = original_qty - used_qty 
        
        if remain_qty > 0:
            # 短缺的數量，需要作為剩餘工單回傳
            # 找到原始工單行作為模板 (取 df_to_process 中該工單的第一行)
            original_row_data = df_paired[df_paired["工單編號"] == key].iloc[0].copy()
            
            remain_row = original_row_data.copy()
            remain_row["預估良品數"] = remain_qty
            remain_row["餘量"] = remain_qty
            # 剩餘工單，刀次設為 0
            remain_row["刀次"] = 0 
            
            new_quantity_remainders.append(remain_row)
        
        # 根據您的要求：「如果數量是比原本多的 就不理他」
        # 我們假設拆分邏輯正確，這裡只處理短缺，不處理超量。

    # 4. 合併所有剩餘量並回傳
    # 獲取原始輸入 df_paired 的欄位順序作為範本
    original_cols_order = df_paired.columns.tolist() 
    
    # 將流程剩餘 (remaining_rows) 和 數量短缺剩餘 (new_quantity_remainders) 合併
    df_new_remainders = pd.concat(
        [pd.DataFrame(remaining_rows), pd.DataFrame(new_quantity_remainders)],
        ignore_index=True
    )

    df_new_remainders_grouped = pd.DataFrame(columns=original_cols_order) # 初始化一個帶有正確欄位順序的空 DataFrame

    if not df_new_remainders.empty:
        
        # 定義聚合方法
        agg_cols = ["預估良品數", "餘量"]
        agg_dict = {col: 'sum' for col in agg_cols}
        
        # 將所有非聚合欄位設為 'first'，並確保只對存在的欄位操作
        for col in df_new_remainders.columns:
            if col not in ["工單編號"] + agg_cols:
                agg_dict[col] = 'first' 

        # 執行聚合 (只對本階段新增的剩餘工單)
        df_new_remainders_grouped = df_new_remainders.groupby("工單編號", dropna=False).agg(agg_dict).reset_index()
        
        # 排除掉良品數為 0 或負數的行
        df_new_remainders_grouped = df_new_remainders_grouped[
            df_new_remainders_grouped["預估良品數"] > 0
        ]
        
        # 【關鍵步驟】：強制將欄位順序恢復為原始順序
        # 由於聚合後可能會遺失一些欄位（例如，如果某欄位完全是 NaN 且未被選為聚合依據），
        # 這裡需要一個穩定的欄位列表，我們使用最原始的欄位列表。
        
        # 確保回傳的 DataFrame 包含所有必要的欄位
        final_cols = [col for col in original_cols_order if col in df_new_remainders_grouped.columns]
        
        # 如果有欄位在 final_cols 但不在 grouped_df，則需要補上 (通常是 '刀次' 等)
        for col in original_cols_order:
             if col not in final_cols and col in df_new_remainders_grouped.columns:
                 final_cols.append(col)
                 
        # 重新選擇欄位並排序
        df_new_remainders_grouped = df_new_remainders_grouped[final_cols]
        
    # ------------------------------------------------------------------
    # 5. 合併總剩餘量並回傳
    # ------------------------------------------------------------------
    
    # 將前一階段傳入的 remaining (保留原始順序) 和 本階段新聚合的剩餘工單 (已恢復欄序) 合併
    df_combined_remaining = pd.concat([remaining, df_new_remainders_grouped], ignore_index=True)

    # 回傳：本階段的排程結果 (used_df) 和 總累積的剩餘量 (df_combined_remaining)
    return used_df, df_combined_remaining
...
def process_schedule_data_second(order_df):
    ...
    # ⬇️ 刀次 > 45 時再拆分 (確保上限為 45)
    expanded_rows = []

    # 定義新的刀次上限
    max_cut = 45
    # exception_max = 60 # 此變數已不需要
    ...
    for _, row in no_pair_df.iterrows():
        try:
            ...
            if cut > max_cut:
                while cut > 0:
                    split_cut = min(cut, max_cut) # 確保切割量不超過 45
                    
                    new_row = row.copy()
                    new_row["刀次"] = split_cut
                    new_row["預估良品數"] = int(car * split_cut)
                    new_row["餘量"] = new_row["預估良品數"]
    ...
    # =========================================================
    # 🎯 針對 remaining_df 進行相同的 45 刀上限拆分
    # =========================================================
    remaining_expanded_rows = []
        ...
        try:
            ...
            if cut > max_cut:
                while cut > 0:
                    ...
                    # 假設這裡只處理主工單（雖然 remaining_df 可能還包含 fractional cut 的資料）
                    # 為了安全，假設這裡的 row 是可獨立計算的
                    new_row["預估良品數"] = int(car * split_cut)
                    new_row["餘量"] = new_row["預估良品數"]
                    remaining_expanded_rows.append(new_row)
                    cut -= split_cut
            
            # 如果刀次小於或等於 45，則直接加入
            else:
                remaining_expanded_rows.append(row)
        except:
            remaining_expanded_rows.append(row)

    remaining_df = pd.DataFrame(remaining_expanded_rows)
    # =========================================================
...
def remaining_cut_clean_and_repair(df_paired_split, df, mode="88cm", cut_limit=45):
    """
    mode: "88cm" → 處理非 B110A
          "68cm" → 處理 B110A
    """
...
def split_pair_orders(main_order, child_order, cut_limit=45):
    """
...
def generate_schedule_for_person(df: pd.DataFrame, break_dates: set, max_lookback_days: int = 45) -> pd.DataFrame:
    df = df.copy()
    ...
    while date >= earliest_start:
        if date.weekday() < 5 and date not in break_dates:
            schedule_capacity[date] = daily_limit
        date -= timedelta(days=1)
    
    # 針對只有一筆主工單的日期，將其上限提升為 60 刀
    for main_idx, group in sorted_groups:
        group_df = df.loc[group.index]
        earliest_end = group_df["組內最早完工日"].min()
        
        # 判斷這個組是否為當天唯一的組
        is_single_order = df[df['預計開工日'] == earliest_end]['主工單索引'].nunique() == 1
        
        # 如果是單一工單，且該天在容量表中，則提高上限
        if is_single_order and earliest_end in schedule_capacity:
            schedule_capacity[earliest_end] = special_limit
...
def split_schedule_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        if not isinstance(sched_str, str) or "(" not in sched_str or total_doz <= 45:
...
def final_cal_list(df: pd.DataFrame, start_d: datetime.date, break_dates: set) -> pd.DataFrame:
    ...
    #special_limit = 60
    special_limit = 45
    # 使用 defaultdict，確保每個日期都有初始值，避免鍵值錯誤
    capacity_used = defaultdict(lambda: {"count": 0, "nonzero_orders_count": 0})
    ...
    for dose in doses:
        ...
        while True:
            ...
            if wd < 5 and current_date not in break_dates:
                ...
                # 取得當天的「已排程」主工單數量
                current_nonzero_orders = capacity_used.get(current_date, {}).get("nonzero_orders_count", 0)

                # 根據新規則決定當日上限
                # 如果當天還沒有主工單，則這筆將會是第一筆，上限為 60
                if current_nonzero_orders == 0:
                    limit = special_limit
                # 如果當天已經有一筆或以上，則上限為 45
                else:
                    limit = daily_limit

                if current_date not in capacity_used:
                    capacity_used[current_date] = {"count": 0, "nonzero_orders_count": 0}

                # 檢查當天容量是否足夠
                if capacity_used[current_date]["count"] + dose <= limit:
                    # 如果排程成功，更新當天的使用量和主工單數量
                    capacity_used[current_date]["count"] += dose
                    capacity_used[current_date]["nonzero_orders_count"] += 1
...
def main():
    ...
    df_paired_split_2, extra_remaining_2 = split_paired_rows_step1(df_paired_split_1, extra_remaining_1)

    # 合併 extra_remaining 並去除重複的工單
    df_remaining = pd.concat([df_remaining, extra_remaining_2], ignore_index=True)
    ...
    # 日期排程 (一天標準刀次55刀，加班可以到65刀，滿足出貨不需要安排加班) -> 改為統一都45刀
    ...
    output_name = os.path.join(script_dir, "排程結果_0930.xlsx")   # 存到同一層資料夾
    ...
    with pd.ExcelWriter(output_name, engine="openpyxl") as writer:
        #df_reamining_second.to_excel(writer, sheet_name="改版剩餘", index=False)
        #df_no_pair_second.to_excel(writer, sheet_name="改版配對完成", index=False)
        #extra_remaining_1.to_excel(writer, sheet_name="初始剩餘1", index=False)
        #extra_remaining_2.to_excel(writer, sheet_name="初始剩餘2", index=False)
        ...
        #final_output.to_excel(writer, sheet_name="test_conbine", index=False)
