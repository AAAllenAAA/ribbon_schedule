...
def process_schedule_data():
    ...
    break_date_list = {
        "A159": set(config_user.get("A159", [])) - {""},
        "A830": set(config_user.get("A830", [])) - {""},
        "B201": set(config_user.get("B201", [])) - {""}
    }
...
def Erin_use2(A830_df, B201_df, A159_df, break_dates_dict):
    """
    專門處理：
    ...
    def add_business_days(start_date, days_to_add, person_name):
        ...
        # 1. 取得該員專屬的休假集合，並強制轉為 date 物件
        raw_breaks = break_dates_dict.get(person_name, set())
        clean_breaks = {pd.to_datetime(d).date() for d in raw_breaks}
        
        curr_date = pd.to_datetime(start_date).date()
        added = 0
        while added < days_to_add:
            curr_date += pd.Timedelta(days=1)
            # 判斷是否為有效工作日 (非週末 且 不在該員休假清單內)
            if curr_date.weekday() < 5 and curr_date not in clean_breaks:
                added += 1
        return curr_date
...
def final_cal_list_person(df: pd.DataFrame, start_d: datetime.date, break_dates: set) -> pd.DataFrame:
    ...
    break_dates = {pd.to_datetime(d).date() for d in break_dates}
    print(break_dates)
    ...
    for dose, sku in zip(doses, sku_list):
        ...
        while True:
            wd = current_date.weekday()
            if wd < 5 and current_date.date() not in break_dates:
...
def final_schedule_list_second(A159_part, A830_part, B201_part, break_dates):
    ...
    if not schedule_A159.empty and schedule_A159["刀次"].notna().any() and (schedule_A159["刀次"] > 0).any():
        ...
        # 丟進排程邏輯（使用 datetime 版本）
        try:
            schedule_A159_1 = final_cal_list_person(schedule_A159, start_d=user_schedule_date, break_dates=break_dates["A159"])
            schedule_A159_2 = sort_by_customer_due_date(schedule_A159_1)
            schedule_A159_3 = final_cal_list_person(schedule_A159_2, start_d=user_schedule_date, break_dates=break_dates["A159"])
        except Exception as e:
            print("發生錯誤:", e)
...
def apply_final_rotation(df_A830, df_B201, df_A159, break_dates):
    """
    【身分互換輪替版 - 休假安全強化】
    邏輯：
    1. 算出工單所屬週次（以 12/30 為基準週 0）。
    2. 定義當週「值班生」(Standby Person)：負責執行 A159 的任務 (81B)。
    3. 安全檢查：若值班生或 A159 當天有人請假，則取消該筆工單的互換。
    """
    # 合併所有資料進行標籤重整
    full_df = pd.concat([df_A830, df_B201, df_A159], ignore_index=True)
    if full_df.empty:
    ...
    # 2. 預處理休假資料 (轉為 date 物件集，提升比對速度)
    clean_breaks = {}
    if isinstance(break_dates, dict):
        for person, dates in break_dates.items():
            clean_breaks[person] = {pd.to_datetime(d).date() for d in dates}

    # 3. 鎖定工單首日週序與出勤日 (確保同 ID 主子工單不失散)
    ...
    def swap_identity(row):
        try:
            week_idx = int(row['週序'])
            target_date = row['預計開工日_dt'].date() # 取得開工當天日期
            standby_person = rotation_order[week_idx % 3]
            current_p = str(row['人員']).strip()

            # 如果當週值班生就是 A159，維持原定計畫
            if standby_person == "A159":
                return current_p
            
            # --- 核心安全防護：檢查兩位當事人當天是否請假 ---
            # 檢查當週值班生 (例如 B201) 是否休假
            standby_on_leave = target_date in clean_breaks.get(standby_person, set())
            # 檢查 A159 是否休假
            a159_on_leave = target_date in clean_breaks.get("A159", set())

            # 若有人請假，則該筆工單不進行身分交換，回傳原始人員
            if standby_on_leave or a159_on_leave:
                return current_p

            # --- 執行交換邏輯 ---
            # A. 原本給 A159 的 (81B) -> 雙方都沒請假，才交給值班生
            if current_p == "A159":
                return standby_person
            
            # B. 原本給該值班生的 (81A) -> 雙方都沒請假，才移交給 A159 代班
            if current_p == standby_person:
                return "A159"
...
def fill_standby_capacity_with_limit(df_A830, df_B201, df_A159, break_dates):
    ...
    # --- 【核心修正點】 ---
    # 2. 檢查值班生今天是否請假
    # 將 break_dates 整理成方便比對的格式
    clean_breaks = {p: {pd.to_datetime(d).date() for d in dates} for p, dates in break_dates.items()}
    
    if today_dt.date() in clean_breaks.get(standby_person, set()):
        print(f"今日 ({today_dt.date()}) 值班生 {standby_person} 休假，不執行補位。")
        # 直接按原樣拆分回傳，不進行後續的 pull_amount 邏輯
    ...
    if not future_df.empty:
        ...
        for _, g_info in future_groups.iterrows():
            current_limit = MAX_KNIFE_MAP.get(len(existing_keys), 39)
...
def do_newsheet_for_81B(all_final_combined):
    ...
    # --- 2. 處理主工單 (刀次 > 0) ---
    main_df = all_81B[all_81B['刀次_num'] > 0].copy()
    
    # 從 order_df 抓取「核發日期」與「開工數量」
    ...
    # --- 3. 處理子工單 (刀次 = 0) ---
    sub_df = all_81B[all_81B['刀次_num'] == 0].copy()
    
    # 準備合併：我們需要以 "唯一主工單ID" 作為連結橋樑
    # 將主工單基礎資訊選出來
    base_info = main_df[[
        '核發日期', '工單編號', '品號', '客戶需求日', 
    ...
    # 將子工單資訊選出來
    sub_info = sub_df[['唯一主工單ID', '工單編號', '品號', '預估良品數']].rename(columns={
        '工單編號': '子工單號碼',
        '品號': '子工單料號',
        '預估良品數': '子工單數量'
    })
    ...
    # --- 4. 水平合併 (Left Join) ---
    # 這樣每一行主工單都會帶上它的子工單資訊
    final_81B_report = base_info.merge(sub_info, on='唯一主工單ID', how='left')
    ...
    final_81B_report = final_81B_report[column_order]

    # 將要比較的欄位轉為數字
    final_81B_report['訂購數量'] = pd.to_numeric(final_81B_report['訂購數量'], errors='coerce').fillna(0)
    final_81B_report['預估良品數'] = pd.to_numeric(final_81B_report['預估良品數'], errors='coerce').fillna(0)

    # --- 6. 定義樣式方法 ---
    def highlight_excess(row):
        # 建立一個與 row 欄位相同的樣式列表，預設不帶樣式
        styles = [''] * len(row)
        
        # 找出 '預估良品數' 與 '訂購數量' 在 row 中的索引位置
        yield_idx = row.index.get_loc('預估良品數')
        
        # 條件判斷：預估良品數 > 訂購數量
        if row['預估良品數'] > row['訂購數量']:
            # 對 '預估良品數' 的儲存格設定底色
            styles[yield_idx] = 'background-color: yellow; color: red; font-weight: bold;'
            
        return styles

    # 使用 Styler 應用樣式
    styled_report = final_81B_report.style.apply(highlight_excess, axis=1)

    # 回傳 Styled 對象，匯出時直接 styled_report.to_excel("file.xlsx") 即可帶顏色
...
def main():
    ...
    A830_new, B201_new, A159_new = apply_final_rotation(A830_part_end, B201_part_end, A159_part_end, break_dates)

    A830_new, B201_new, A159_new = fill_standby_capacity_with_limit(A830_new, B201_new, A159_new, break_dates)

    A830_new, B201_new, A159_new = Erin_use2(A830_new, B201_new, A159_new, break_dates)
    ...
    target_date_str = pd.to_datetime(user_schedule_date).strftime("%Y-%m-%d")
    day_mask = all_final_combined['預計開工日'] == target_date_str
