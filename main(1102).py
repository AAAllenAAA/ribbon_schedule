...
def prepare_final_schedule(df_paired_split, df_no_pair_split, df_no_pair_second, final_remaining, df_history):
    ...
    for pos in range(len(df_paired_split)):
        ...
        if row["刀次"] != 0:
            ...
            if prev_main.empty:
                df_paired_split.at[df_paired_split.index[pos], "主工單編號"] = None
            else:
...
def final_doAssignAndSort(df_paired_split, df_no_pair_split, df_no_pair_second, remaining):
    ...
    for idx, row in remaining.iterrows():
        ...
        if main_car_count <= 0:
            main_car_count = 1
        if pd.isna(original_qty) or pd.isna(main_car_count) or main_car_count == 0:
            print(f"⚠️ 警告：工單 {sn} 缺少數量資料（original_qty={original_qty}, main_car_count={main_car_count}），以 0 處理。")
            main_cut_count = 0
        else:
...
def main():
    ...
    A830_part_end_list, B201_part_end_list, A159_part_end_list = final_schedule_list(A159_part, A830_part, B201_part)
    ...
    output_name = os.path.join(script_dir, "排程結果_1030(1031).xlsx")   # 存到同一層資料夾
